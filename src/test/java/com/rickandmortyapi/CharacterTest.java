...
import com.rickandmorty.Character;
...
public class CharacterTest {
	...
	public void testGet_withIds5and10() {
		final Character jerrySmith = character.get(5, 10).iterator().next();
	...
	public void testFilter_withRickAlive() {
		for (Character rick : character.withName("rick").withStatus(Character.Status.ALIVE).filter()) {
	...
	public void testFilter_withAnnet() {
		assertTrue(character.withName("annet").filter().isEmpty());
	}
